...
int fas_set_stream_info(const char* host, int port, const char* mount, const char* password) {
    shout_set_host_wrapper(g_shout, host);
    shout_set_port_wrapper(g_shout, port);
    shout_set_mount_wrapper(g_shout, mount);
    shout_set_password_wrapper(g_shout, password);
    return 0;
}
int fas_start() {
